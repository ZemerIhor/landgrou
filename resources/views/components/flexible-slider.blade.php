...
<div class="relative w-full overflow-hidden">
    <div class="swiper {{ $swiperClass }}" data-swiper-config='@json($config)'>
        <div class="swiper-wrapper flex gap-5 items-center mt-5 w-full" role="region" aria-label="{{ $ariaLabel ?? 'Галерея зображень' }}">
            {{ $slot }}
        </div>

        <div class="swiper-button-prev {{ $prevClass }} absolute right-6 bottom-0 -translate-y-1/2 z-10 bg-white rounded-full shadow-md p-2 hover:scale-110 transition">
        ...
        <div class="swiper-button-next {{ $nextClass }} absolute right-0 bottom-0 -translate-y-1/2 z-10 bg-white rounded-full shadow-md p-2 hover:scale-110 transition">
