    ...
    <div class="relative mx-auto transition-opacity duration-300" style="width: 90%" id="slider-wrap">
        <div class="swiper-container" id="hero-swiper">
            <div class="swiper-wrapper">
                ...
                @if (!empty($heroSlides) && is_array($heroSlides))
                    @foreach ($heroSlides as $index => $slide)
                        <div class="swiper-slide w-full justify-center rounded-2xl items-center bg-black bg-cover bg-center"
                    ...
                    @endforeach
                @else
                    <div class="swiper-slide min-w-full flex items-center justify-center bg-gray-200 text-gray-500 p-10">
            ...
            @if (!empty($heroSlides) && is_array($heroSlides) && count($heroSlides) > 1)
                <div class="swiper-button-prev absolute top-1/2 left-4 transform -translate-y-1/2 rounded-full p-2">
                ...
                <div class="swiper-button-next absolute top-1/2 right-4 transform -translate-y-1/2 rounded-full p-2">
                    <svg width="15" height="46" viewBox="0 0 15 46" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="swiper-pagination absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2"></div>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const heroSwiper = new Swiper('#hero-swiper', {
            initialSlide: 1, // Start with the 2nd slide (0-based index)
            slidesPerView: 1,
            spaceBetween: 20,
            ...
            pagination: {
                ...
                clickable: true,
                bulletClass: 'w-4 h-1 rounded-full cursor-pointer bg-white/50',
            ...
            on: {
                init: function () {
                    ...
                    requestAnimationFrame(() => {
                        sliderWrap.classList.add('opacity-100', 'visible');
                    });
                },
            },
        });
        ...
        document.addEventListener('livewire:navigated', () => {
            console.log('Livewire navigated, reinitializing hero swiper');
            if (heroSwiper) {
                heroSwiper.destroy(true, true);
            }
            // Reinitialize Hero Swiper
            const newHeroSwiper = new Swiper('#hero-swiper', {
                initialSlide: 1,
                slidesPerView: 1,
                spaceBetween: 20,
                centeredSlides: true,
        ...
        // Assuming initializeReviewSwiper() is defined elsewhere
