<section class="flex mx-auto flex-col self-stretch px-2 py-12 container relative" role="main"
         aria-labelledby="blog-heading">
    <h1 id="blog-heading" class="text-2xl font-bold leading-tight text-zinc-800 max-md:max-w-full">
        Блог
    </h1>

    <div class="flex flex-wrap gap-2 items-center mt-5 w-full min-h-[307px] max-md:max-w-full" role="region"
         aria-label="Статті">
        @forelse ($posts->take(4) as $post)
            ...
            <a href="{{ $postUrl }}"
               wire:navigate
               class="block overflow-hidden relative flex-1 shrink self-stretch my-auto rounded-3xl basis-0 bg-neutral-200 min-w-60 hover:shadow-lg transition-shadow h-full"
               aria-label="Читати статтю: {{ $post->getTranslation('title', $locale) }}">
                <article class="overflow-hidden" role="article">
                    <div class="overflow-hidden z-0 w-full">
                        <img src="{{ $post->banner ? Storage::url($post->banner) : 'https://via.placeholder.com/300x158' }}"
        ...
        @empty
            <p class="text-sm text-neutral-400 w-full text-center">Немає постів</p>
        @endforelse
    </div>
