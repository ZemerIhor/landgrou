<section class="flex mx-auto flex-col self-stretch md:px-12 px-4 py-12 container relative" role="main" aria-labelledby="blog-heading">
    ...
    <div class="flex flex-wrap gap-2 items-center mt-5 w-full min-h-[307px] max-md:max-w-full" role="region" aria-label="{{ __('blog.articles') }}">
        @forelse ($blogPosts->take(4) as $post)
            @php
                ...
                $slug = $post->slug; // Предполагаем, что slug не переводимый
