...
<div x-data="{ isScrolled: false, mobileMenu: false, languageMenu: false, headerHeight: 0 }"
     x-init="headerHeight = document.getElementById('header-container').offsetHeight; $watch('isScrolled', () => headerHeight = document.getElementById('header-container').offsetHeight)"
     @scroll.window="isScrolled = (window.scrollY > 0)"
     @resize.window="headerHeight = document.getElementById('header-container').offsetHeight">
    <div id="header-container" class="relative">
        <header id="header"
                class="fixed top-0 left-0 right-0 z-50 bg-white shadow-xl flex items-center transition-all duration-300"
                role="banner">
            <div class="nav-header flex relative px-4 py-4 justify-between items-center w-full container mx-auto">
                <!-- Logo -->
                ...
                <style>
                    ...
                    /* Hide desktop menu on screens below 900px */
                ...
                <div class="nav-right">
                    ...
                    <div class="flex gap-2 items-center">
                        <a href="{{ isset($headerSettings->instagram_url[$locale]) ? $headerSettings->instagram_url[$locale] : 'https://instagram.com' }}"
                           ...
                           aria-label="{{ __('messages.social.instagram') }}">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="social-icon">
                    ...
                    <div
                        ...
                        x-show="mobileMenu"
                        id="mobile-menu"
                        class="bg-white shadow-lg absolute top-full left-0 w-full z-50"
                    ...
                    >
                        <nav class="flex flex-col items-center gap-4 px-4 py-6 text-base font-semibold text-zinc-800" role="navigation" aria-label="{{ __('messages.nav.mobile_navigation') }}">
                            <!-- Menu Items -->
                            @if ($headerMenu)
                                <ul class="mobile-menu-items w-full">
                                    @foreach ($headerMenu->menuItems as $item)
                                        <li>
                                            <a href="{{ $item->url }}" class="w-full text-center py-2 hover:text-green-600 block">
                                                {{ $item->title }}
                                            </a>
                                            @if ($item->children)
                                                <ul class="mobile-menu-items">
                                                    @foreach ($item->children as $child)
                                                        <li>
                                                            <a href="{{ $child->url }}" class="w-full text-center py-2 text-sm hover:text-green-600 block">
                                                                {{ $child->title }}
                                                            </a>
                                                        </li>
                                                    @endforeach
        ...
        <div class="header-placeholder"
             ...
             x-transition:enter-end="height: ' + headerHeight + 'px'"
             x-transition:leave="transition ease-in duration-300"
             x-transition:leave-start="height: ' + headerHeight + 'px'"
